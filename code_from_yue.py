...
def code():
    ...
    def write_to_to_file(tank_number, battery_level,water_level):
        ...
        if my_file.is_file() == False:
            with open(file_name,'w') as csvfile:
                ...
                for num in range(0,144):
                    writer.writerow(['#'+current_time,'-100','-100'])
                csvfile.close
        ...
        try: 
            ...
            current_row = counter[counter_number]%144+1
            line_to_override = {current_row:[current_time,battery_level,water_level]}    
    ...
    port = 'COM3'
    baud_rate = 9600
    port_file = Path("port_brand.txt")
    if port_file.is_file():
        with open('port_brand.txt') as f:
            lines = f.readlines()
            port = lines[0].strip()
            baud_rate = int(lines[1].strip())
    else:
        print("enter port and baud rate from GUI first")
